...
Supported receivers are exactly `Illuminate\Routing\Router` for `get`, `post`, `put`, `patch`, `delete`, `options`, `any`, `match`, and `addRoute`, and exactly `Illuminate\Routing\Route` for `setUri`. The same router methods are checked through the native `Illuminate\Support\Facades\Route` facade when its unmodified `router` accessor and the corresponding native router method can both be proven from source. Fully qualified names, lexical class imports, renamed imports and namespace imports are supported; local classes and runtime aliases defer. Positional and named `uri` arguments are supported. Optional markers and binding fields are normalized for comparison: `{item:slug}` and `{item:id?}` use the same parameter name. Names are case sensitive.

This is a declaration-level warning based on the URI at that call, not a simulation of the final route collection. Analysis never boots Laravel, evaluates application PHP, or queries a database. The analyzed source is parsed to resolve lexical names and targeted calls; there is no filesystem route registry scan.

Dynamic expressions, argument unpacking, subclasses, overridden methods, custom facade accessors, and facade methods that cannot be traced to the native router are deferred. Only conventional ASCII placeholder and binding-field names are checked. Group prefixes, domains, later route mutations, and provider registrations are not reconstructed. Named-route existence and middleware alias/class validity are not checked: container/provider registrations can supply them dynamically.
